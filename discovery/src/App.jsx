...
function App() {
  ...
  const apiKey = 'kYcU7OgmDWGqybLpkeFZuYQLg2cME0QL9HltFW5G'; // Your NASA API key
      ...
      <div className='photo-container'>
        <div className='buttons'>
          ...
          <p>
            ...
            <button onClick={() => addToBanList(photo.camera.name)}>DiscoveryBan</button>
          </p>
        </div>
        <button onClick={fetchRandomPhoto}>Discovery New Random Photo</button>
      </div>
      <h2> Discovery Banned Attributes</h2>
      <ul>
        {bannedAttributes.map((attribute, index) => (
          <li key={index}>
            {attribute}{' '}
            <button onClick={() => removeFromBanList(attribute)}>Discovery Unban</button>
          </li>
        ))}
      </ul>
    </div>
